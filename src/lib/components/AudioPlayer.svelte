...
<div class="my-1 flex w-full flex-col gap-3 rounded-md bg-white px-3 py-3 shadow-md">
	<div class="flex gap-2">
		...
		<div id="waveform" class="relative w-full py-1">
			<div
				id="time"
				class="pointer-events-none absolute inset-y-3/4 left-0 z-10 size-min w-9 rounded-tr-md bg-white/90 px-1 text-sm whitespace-nowrap"
			>
				<p class="font-syne-mono">{formattedTime}</p>
			</div>
			<div
				id="duration"
				class="pointer-events-none absolute inset-y-3/4 right-0 z-10 size-min w-9 rounded-tl-md bg-white/90 px-1 text-sm"
			>
				<p class="font-syne-mono">{formattedDuration}</p>
			</div>
		</div>
	</div>
	...
	<div class="flex items-center gap-2">
		<div class="flex items-center gap-1">
			<button title={volume ? 'Mute' : 'Unmute'} onclick={muteButton}>
				{#if volume >= 0.5}
					<svg
						class="text-darkgrey h-5 w-5"
						aria-hidden="true"
						xmlns="http://www.w3.org/2000/svg"
						width="24"
						height="24"

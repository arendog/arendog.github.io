<script lang="ts">
	...
	const sketch: Sketch = (p) => {
		...
		p.setup = async () => {
			...
			p.textSize(72);
			const contours1 = fontTactile.textToContours('alex rennie', 5, 60, { sampleFactor: 0.5 });

			p.textSize(48);
			const contours2 = fontMono.textToContours('composer', 60, 120, { sampleFactor: 0.5 });
		...
		p.draw = () => {
			...
			for (const points of contours) {
				...
				for (const point of points) {
					...
					p.vertex(point.x + p.cos(incFast) * AMPLITUDE_X, point.y + p.sin(incFast) * AMPLITUDE_Y);
				}
...
<a
	...
	class="flex min-h-screen min-w-screen items-center justify-center"
	rel="external"
	href="/about"
>
	<P5Sketch {sketch} />
